# SONGSS Intelligence — CLAUDE_ARCHIVE.md
> Historical / resolved-work archive, split out of CLAUDE.md on 2026-08-24
> because CLAUDE.md had grown to 236k chars (>150k auto-load limit), costing
> tokens on every turn. This file is NOT auto-loaded — it is the full,
> verbatim record of every RESOLVED / IMPLEMENTED / investigated entry and
> every struck-through task as they existed on 2026-08-24. CLAUDE.md now keeps
> only live pointers + open tasks and points here (and to memory/*.md) for
> full detail. Nothing was deleted; grep this file for any past decision,
> lesson, or gotcha.
...
RESOLVED (2026-08-22): **GoTrue SMTP fixed — "Forgot password?" now
delivers a real email.** `GOTRUE_SMTP_*` in `/root/supabase/.env` had been
pointing at a non-existent `supabase-mail` container with fake placeholder
credentials since this stack was first set up — password-recovery and
email-change confirmation links were silently undeliverable the entire
time. Found while scoping the `/settings` Change Email feature; this
session applied the fix.

**Confirmed before touching anything**: `docker-compose.yml`'s `auth`
service already does `GOTRUE_SMTP_HOST: ${SMTP_HOST}` etc. — only `auth`
references `${SMTP_*}`, no compose structural change needed, only `.env`
values. Backup taken first
(`.env.backup_20260822_115820_pre_gotrue_smtp_fix`).

**Real finding**: Gilberto's regular Zoho account-login password does NOT
work for SMTP AUTH — Zoho rejected it with a genuine `535 Authentication
Failed` (confirmed via `docker logs supabase-auth`, not just a generic API
500), the standard Zoho behavior when an app-specific password is required.
Fixed by reusing n8n's already-proven Zoho SMTP credential (id
`6JTe1tM0OaFntxjN`, same `smtp.zoho.com`/`admin@songssintelligence.com`,
already sending real welcome/report emails in production) — extracted
container-to-host via `n8n export:credentials --decrypted` → a `node -e`
filter pulling only the `password` field → `docker cp` → a Python script
that upserted it into `.env` and shredded the source, the value never
printed at any step.

**Operational gotcha found, now a standing lesson**: after
`docker compose up -d --force-recreate auth` (both attempts), Kong's
proxied requests to `auth` hung and eventually 502'd, with zero
corresponding entries in `auth`'s own logs — the recreated container itself
was `healthy`/`RestartCount=0` throughout, so this was Kong holding a stale
upstream connection, not an auth problem. Fixed with a plain
`docker compose restart kong` (no env change needed). **Going forward:
after force-recreating any service Kong proxies to, if requests start
hanging/502ing even though the recreated service reports healthy, restart
Kong too** — don't assume the just-changed service is at fault.

**Live-verified twice**: a disposable GoTrue admin-API user
(`hello+test-smtp-fix-<timestamp>@songssintelligence.com`) + a real
`POST /auth/v1/recover` → clean `200`, ~1.9s duration (consistent with a
real synchronous SMTP send, vs. ~0.1s for the earlier rejection), zero
error log lines; **Gilberto independently confirmed the email actually
arrived** in the real `hello@` inbox with a working reset link. All
disposable test users (including one orphaned by the mid-investigation Kong
hang) deleted after, 0 left; all scratch scripts/temp files removed or
shredded.

**Incidental finding, flagged not auto-rotated**: getting the real Zoho
password into `.env` hit a real environment limitation — hidden-prompt
`getpass()` doesn't work through the `!` exec prefix (no TTY, immediate
`EOFError`), so Gilberto ran it from a separate real SSH terminal instead.
Directly hand-editing `.env` in a later step then caused the harness's own
"file changed outside my edits" mechanism to surface that diff — Gilberto's
typed Zoho account password — into the session in plaintext. That value is
no longer live anywhere (superseded by the n8n-sourced password before ever
being used successfully), but per this project's standard for any secret
that reaches a transcript, Gilberto may want to rotate that Zoho account
password as a precaution. See
`feedback_gotrue_getpass_no_tty_fallback_exposure` in memory for the full
mechanism and how to avoid it next time.

**Not done, out of scope**: `GOTRUE_MAILER_SECURE_EMAIL_CHANGE_ENABLED`
stays commented out (GoTrue default `true`/double-confirm applies) —
confirmed present, only relevant if/when the deferred Change Email feature
is built. Port 465 worked cleanly on the first real attempt after the
correct password — no need for the 587+STARTTLS fallback.

DISCOVERED UNDOCUMENTED (built 2026-08-22, written up 2026-08-23): **"Content
Automation — Video Captioning"** — a new, separate n8n workflow (id
`contentAutomationCaption01`), fully disconnected from the NIE/Stripe
pipeline (no Supabase writes, no shared credentials). Found only because this
session was asked to update its Drive folder ID and the workflow wasn't
listed anywhere in this doc or in memory — same "built but never written
up" gap as the 2026-08-20 synthetic monitoring system (see §4 above). A
backup file (`manual_20260822_224634_pre_content_automation_workflow.sqlite`)
confirms it was actually built 2026-08-22, one day before this entry.
**Currently inactive** (`active: false`) and **has no credentials attached
on any node yet** — Gilberto is connecting them directly via the n8n UI as
the next step (Google Drive OAuth2 via the new SONGSS Gmail account project,
and an Anthropic credential for the Claude call).

**Purpose**: watches a Google Drive folder for newly-uploaded marketing
video files, generates a short on-brand social caption for each via Claude,
and saves the caption back to Drive as a `.txt` file alongside the video.

**Nodes (5, linear chain, no branching except the video-type filter)**:
1. `Watch for New Video` (`googleDriveTrigger`) — polls every 5 minutes,
   `triggerOn: specificFolder`, `event: fileCreated`. No credential attached
   yet.
2. `Is Video File?` (`if`) — `$json.mimeType` must start with `"video/"`;
   non-video uploads to the same folder (e.g. a stray caption `.txt` file)
   correctly stop here rather than looping back into captioning.
3. `Build Caption Prompt` (`code`) — strips the file extension from the
   uploaded filename and builds a prompt instructing the model to write a
   sub-280-character English caption for SONGSS Intelligence marketing
   content, explicitly told not to invent facts/numbers/claims beyond what
   the title implies.
4. `Generate Caption (Claude)` (`@n8n/n8n-nodes-langchain.anthropic`) —
   `model: claude-haiku-4-5-20251001`, `maxTokens: 300`,
   `temperature: 0.9`, same "never invent specific facts" instruction
   repeated in the system prompt. No credential attached yet.
5. `Save Caption to Drive` (`googleDrive`, `createFromText`) — writes
   `{baseName} - caption.txt` back into the **same** watched folder (not a
   separate output folder), content = Claude's raw response text. No
   credential attached yet.

**Google Drive folder ID — updated this session**: both `Watch for New
Video`'s `folderToWatch` and `Save Caption to Drive`'s `folderId` were
repointed from an old folder (`1awDdtCJ1LDSZjuw5r1DaPZU5Ut96q-eU`, tied to
the original Google account) to the new folder **"ParaPostar Songss NIE"**
(`1NSQyPD8e8bYLJdFY2zxPg8RdZaUDSJfs`), since the team switched to a
different Google account than originally planned. Deployed via this
project's standard direct-SQL patch method (backup first —
`manual_20260823_013108_pre_content_automation_drive_folder_update.sqlite`
— dry run against a scratch copy, patched both `workflow_entity.nodes` and
the workflow's single `workflow_history` row, `docker restart n8n_songss`,
export-diff confirmed exact scope: same 5 nodes, only the two folder ID
values changed). Since the workflow is inactive and shares no state with
the live NIE/Stripe pipeline, this carried zero risk to production traffic.

**Known dependency, not yet satisfied**: repointing the folder ID does not
by itself grant access — whichever Google account backs the eventual Drive
OAuth2 credential must actually have access to the new "ParaPostar Songss
NIE" folder (shared with it, or the folder must live in that account's own
Drive) or both Drive nodes will fail at runtime once activated. Not
verified this session since no credential exists yet.

**Not done yet**: no credentials on any of the 3 credential-bearing nodes
(`Watch for New Video`, `Generate Caption (Claude)`, `Save Caption to
Drive`) — Gilberto is wiring these up directly in the n8n UI (Google Drive
OAuth2 using the new SONGSS Gmail account project's Client ID/Secret, and
an Anthropic API credential) as the immediate next step. Workflow remains
inactive until that's done and end-to-end tested.
...
- [ ] Content Automation — Video Captioning workflow (`contentAutomationCaption01`)
      — Drive folder ID repointed to the new "ParaPostar Songss NIE" folder
      (`1NSQyPD8e8bYLJdFY2zxPg8RdZaUDSJfs`) 2026-08-23, see §4 "DISCOVERED
      UNDOCUMENTED (built 2026-08-22, written up 2026-08-23)". Still needed
      before this can go live: (1) Gilberto to connect the Google Drive
      OAuth2 credential (new SONGSS Gmail account project's Client
      ID/Secret) on `Watch for New Video` + `Save Caption to Drive`, and an
      Anthropic credential on `Generate Caption (Claude)` — in progress,
      via the n8n UI directly; (2) confirm that Drive account actually has
      access to the new folder; (3) end-to-end test (upload a real video,
      confirm a caption `.txt` lands back in the folder) before activating.
...
- [x] ~~Supabase `JWT_SECRET`/`ANON_KEY`/`SERVICE_ROLE_KEY` rotation #6~~
      COMPLETED 2026-09-17, but never documented at the time — reconstructed
      retroactively during rotation #7's session (2026-09-19) from file-system
      evidence: `.env.backup_20260917_150655_pre_jwt_rotation6` (backend),
      `secrets.env.backup` same-day (n8n), and a distinct
      `.env.backup_20260917_210713_pre_jwt_rotation6_frontend` roughly 6
      hours later (frontend) — three separate backup timestamps matching the
      3-leg pattern of every prior rotation. Gilberto confirmed verbally it
      completed; this environment independently corroborated it before
      trusting that confirmation, via `iat`-claim comparison only (never by
      re-printing any key): `/root/supabase/.env` (`ANON_KEY`/
      `SERVICE_ROLE_KEY`), `/docker/n8n/secrets.env`
      (`SUPABASE_SERVICE_ROLE_KEY`), all 9 hardcoded NIE-workflow `apikey`
      nodes, the Geo Activity workflow's 1 node, and
      `/root/songss-intelligence/.env`'s `VITE_SUPABASE_PUBLISHABLE_KEY` all
      shared one `iat`; 3 real successful n8n executions
      (`#2043`-`#2045`, 21:25-21:26 UTC 09-17) immediately after the backend
      leg confirmed the n8n credential (`Aqlm0Ocboq2dZEIl`) was in sync too.
      No further action taken on #6 itself — this entry exists purely so a
      future session doesn't hit the same "undocumented rotation" gap again.
- [x] ~~Supabase `JWT_SECRET`/`ANON_KEY`/`SERVICE_ROLE_KEY` rotation #7~~
      RESOLVED 2026-09-19. Triggered mid-session (real Checkpoint 3 payment-
      architecture implementation work, see
      `project_payment_account_architecture_audit_2026-09-19` for what led
      here): reading the live `Insert Intelligence Report` node's full
      `parameters` JSON to understand the Supabase-write pattern printed its
      hardcoded `apikey` header in the transcript in full — the exact same
      class of mistake that forced #5, this time self-caught rather than
      found during an unrelated bug investigation. Gilberto's call: rotate
      immediately, same as #5's precedent, and good timing since checkouts
      are closed (no real customer session to invalidate).

      **Pre-flight**: discovered rotation #6 was real but undocumented (see
      entry above) — verified it before treating it as the baseline, per
      `feedback_verify_before_trusting_diagnosis`'s general discipline
      applied to the user's own recollection, not just a third-party tool.

      **Done, live-verified, same pattern as #1-6**: backups first (`.env`
      x3 — supabase/n8n secrets/both frontend repos —, `pg_dumpall`, n8n
      sqlite `.backup`, pre-patch exports of both the NIE and Geo Activity
      workflows), all timestamped `20260919_210247`. New
      `JWT_SECRET`/`ANON_KEY`/`SERVICE_ROLE_KEY` generated by a script that
      self-verified its own HS256 implementation by reproducing the live
      `ANON_KEY`'s actual signature bytes under the live secret before
      trusting it to mint new ones (same discipline as #4's fixed method);
      new keys sanity-checked to verify under the new secret and fail under
      the old. `docker compose up -d --force-recreate db auth rest storage
      meta analytics studio kong`. **Live-verified**: new anon key → real
      `200` on `/auth/v1/health` and `/rest/v1/plan_limits`; old anon key →
      real `401` negative control. n8n's `SUPABASE_SERVICE_ROLE_KEY`
      (`/docker/n8n/secrets.env`) patched, `docker compose up -d
      --force-recreate n8n`. Credential `Aqlm0Ocboq2dZEIl` updated via
      `n8n import:credentials` (built from a fresh `--decrypted` export,
      only its `data.value` field touched), re-exported and SHA-256-compared
      to confirm it matches the intended new value and differs from the old
      — no raw value ever printed. All 9 NIE-workflow `apikey` headers
      + the Geo Activity workflow's 1 node patched via the standard
      3-location/2-location method, matched by decoding each header's `role`
      claim (`service_role` only, dynamic `={{ $env... }}` expressions
      correctly left untouched) rather than a blind old-value replace;
      live `versionId`/`activeVersionId` re-verified fresh rather than
      trusted from this doc (NIE: `versionId 59e43853-...` / `activeVersionId
      a09c4898-...`, unchanged from the 2026-09-14 edit; Geo Activity:
      `c8211b90-...`, unchanged). `docker restart n8n_songss`. Export-diff
      confirmed: 71→71 / 5→5 nodes, `connections` byte-identical, exactly
      the expected 9 + 1 nodes changed and nothing else.

      **Live end-to-end verification — via the standing synthetic monitor,
      not a hand-built test**: `watch_n8n_restart.sh` (§3) fired
      automatically on both n8n restarts this rotation caused. The *first*
      firing (`cs_test_synthetic_monitor_20260919_210901`) genuinely
      **failed** — "no auth.users row appeared... Failure is at or before
      the 'Create Supabase User' node" — and paged Telegram, because that
      restart landed in the real window where GoTrue already had the new
      `JWT_SECRET` but the n8n credential still held the old
      `SERVICE_ROLE_KEY` (credential update hadn't happened yet in the
      sequence). Expected given the timing, not a rotation defect; the
      *second* firing after the credential + node patches
      (`cs_test_synthetic_monitor_20260919_211302`) passed clean end-to-end
      (`report_markdown=3285` chars, `processed_sessions` row present, exit
      0). Confirms the monitor itself is a reliable live tripwire for
      exactly this class of mid-rotation gap.

      **Frontend legs**: both `/root/songss-intelligence/.env` and
      `/root/songss-landing-page/.env` patched
      (`VITE_SUPABASE_PUBLISHABLE_KEY`), both rebuilt, both bundles
      byte-confirmed to contain the new key and not the old (checked via a
      script reading the key from a 600-perm file, never inlined in a
      command). Deploy itself (`wrangler deploy` for both `songss-app` and
      the landing-page Worker) left to Gilberto — offered a scoped
      Cloudflare token exchange per Golden Rule 7, he chose to run both
      deploys himself instead. **Open until he confirms**: both Workers are
      still serving the pre-rotation anon key until that deploy happens —
      note this key is a real `401` against the rotated stack, not a
      harmlessly-still-valid credential (see #8 below for the corrected
      framing; this entry's original "low risk since RLS/quota logic
      doesn't care which valid anon key" line was imprecise).

      All plaintext-key scratch files (key-generation output, credential
      export/import JSON, pre-rotation node-JSON dumps) shredded (`shred
      -u`) immediately after use, including inside the n8n container's
      `/tmp`.

      **Superseded same session by #8 before Gilberto ever got to deploy
      this** — see immediately below.
- [x] ~~Supabase `JWT_SECRET`/`ANON_KEY`/`SERVICE_ROLE_KEY` rotation #8~~
      RESOLVED 2026-09-19, same session as #7. Cause: while designing the
      new subscription-lifecycle n8n nodes right after finishing #7, printed
      `Insert Intelligence Report`'s full node JSON again (as a template
      reference) — exposing #7's brand-new key the identical way that forced
      #7 in the first place. Self-caught and disclosed immediately;
      Gilberto's response codified a permanent fix rather than just
      approving another rotation: **redact known secret-bearing fields
      (`apikey`/`authorization`/`password`/`secret`/`token`, case-
      insensitive) before ever displaying node JSON, unconditionally** — not
      just when checking a value (the #5-era `iat`/`role`-only-decode habit
      already covered that case; it didn't cover "browsing structure,"
      which is what caused both #7 and #8). See
      `feedback_redact_node_json_before_display`.

      Identical procedure to #7, executed back to back: backups (`.env` x3,
      `pg_dumpall`, n8n sqlite `.backup`, both workflow exports,
      `20260919_220501`), new keys generated + HS256-self-verified against
      the live (rotation-7) `ANON_KEY`'s actual signature bytes,
      `docker compose up -d --force-recreate db auth rest storage meta
      analytics studio kong`, live-verified (new key `200` on
      `/rest/v1/plan_limits` + `/auth/v1/health`, old key real `401`
      negative control). n8n `SUPABASE_SERVICE_ROLE_KEY` patched,
      `--force-recreate n8n`, credential `Aqlm0Ocboq2dZEIl` rebuilt from a
      fresh `--decrypted` export and SHA-256-verified to match the intended
      new value and differ from the old (no raw value printed). Live
      `versionId`/`activeVersionId` re-checked (unchanged from #7:
      `59e43853-...`/`a09c4898-...` NIE, `c8211b90-...` Geo Activity — raw
      SQL patches don't move these). All 9 + 1 `apikey` headers re-patched
      via the same role-decode-matched method, export-diffed: 71→71 / 5→5
      nodes, connections byte-identical, exactly the same 9 + 1 nodes
      changed.

      **Live end-to-end verification, again via the standing synthetic
      monitor**: both restarts this rotation caused (secrets.env
      force-recreate + post-node-patch restart) landed inside one debounce
      window and were coalesced into a single test per the watcher's own
      design, dispatched after the second settled
      (`cs_test_synthetic_monitor_20260919_221201`). Passed clean
      end-to-end on that one firing — stage 1 (account creation, 5s) and
      stage 2 (`report_markdown=3276` chars, `processed_sessions` row
      present) both green, exit 0, no failure this time. Confirms the
      credential update landed before the final restart in this rotation's
      sequencing, unlike #7's first (expected, explained) failure.

      **Frontend legs**: both `.env` files patched, both bundles rebuilt and
      byte-confirmed (new key present, old absent) — same as #7, this time
      superseding #7's not-yet-deployed bundles before Gilberto ever ran
      that deploy, so no wasted deploy of an already-dead key. Deploy again
      left to Gilberto (declined the offered Cloudflare-token exchange a
      second time). **Open**: same as #7's open item, now against #8's key
      instead — confirm next session whether the deploy happened.

      All plaintext-key scratch files shredded immediately after use, same
      discipline as #7.
